...
function registerAccountabilityRoutes(app, deps) {
    ...
    app.get("/api/accountability/inbox", requireSupabase, requireUser, async (req, res, next) => {
        try {
            ...
            const names = new Map(await Promise.all(actorIds.map(async (id) => [id, await displayNameForUser(id)])));
            const items = [
                ...(notifications.data || []).map((row) => ({ kind: "attempt", ...row, attempt: byId.get(row.attempt_id), actorDisplayName: names.get(byId.get(row.attempt_id)?.actor_user_id) || "Friend" })),
                ...(messages.data || []).map((row) => ({ kind: "message", ...row, attempt: byId.get(row.attempt_id) }))
